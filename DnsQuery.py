...
class ThreadingDnsQuery(threading.Thread):
    ...
    def _send_request_standard(self, request):
        if self.tcp == False:
            try:
                response = dns.query.udp(request, self.dns_server, timeout=self.timeout, port=self.dns_server_port, source=self.src_ip, source_port=self.src_port)
                return response
            except:
        ...
        if src_ip_list == None:
            if src_port_list != None:
                ...
                for i in src_port_list:
                    self._new_thread('Thread-' + str(thread_num), self._generate_request(record_type, rdclass, domain_name, recurse), src_port=i)
                    thread_num += 1

            else:
                dump_info('Generating threads...')
                for i in xrange(concurrent_threads):
                    self._new_thread('Thread-'+str(i), self._generate_request(record_type, rdclass, domain_name, recurse))
            ...
            if src_port_list != None:
                ...
                for j in src_ip_list:
                    for k in src_port_list:
                        self._new_thread('Thread-' + str(thread_num), self._generate_request(record_type, rdclass, domain_name, recurse), src_ip=j, src_port=k)
                        thread_num += 1
